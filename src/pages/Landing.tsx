...
import { LandingLoadOverlay } from "@/components/site/LandingLoadOverlay";
...
const Landing = () => {
  const { hero, features: feat, about, loading: contentLoading } = useLandingSiteContent(
    heroDefaults,
    featuresDefaults,
    aboutDefaults,
  );
  ...
  return (
    <div className="relative min-h-screen">
      <div
        className={
          showLoadOverlay
            ? // Cap height + pause decorative motion while CSS-filtering — cheaper than blurring a tall animated tree
              "min-h-screen max-h-screen overflow-hidden bg-background flex flex-col blur-sm pointer-events-none select-none"
            : "min-h-screen bg-background flex flex-col"
        }
        aria-hidden={showLoadOverlay ? true : undefined}
      >
        <SiteNav />
        ...
        <section className="relative overflow-hidden">
          ...
          <div
            className={
              "absolute -top-32 -left-32 w-[28rem] h-[28rem] bg-primary/10 rounded-full blur-3xl" +
              (showLoadOverlay ? "" : " animate-blob")
            }
          />
          <div
            className={
              "absolute -bottom-40 -right-32 w-[26rem] h-[26rem] bg-accent/15 rounded-full blur-3xl" +
              (showLoadOverlay ? "" : " animate-blob [animation-delay:-6s]")
            }
          />
            ...
            <div className="animate-fade-in order-2 lg:order-1 lg:col-span-6">
              <div className="relative mx-auto max-w-sm sm:max-w-md md:max-w-xl lg:max-w-none">
                ...
                <img
                  ...
                  width={1280}
                  height={896}
                  loading={showLoadOverlay ? "lazy" : "eager"}
                  decoding="async"
                  className={
                    "relative w-full aspect-[4/3] object-cover rounded-2xl sm:rounded-3xl shadow-elegant border-2 sm:border-4 border-card" +
                    (showLoadOverlay ? "" : " animate-float-slow")
                  }
                />
              </div>
            </div>
